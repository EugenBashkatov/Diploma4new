...
def build_graph_with_clusters(start_point, data_list, DEBUG=False):
    ...
    # -----------------------------------------------
    d_cluster_begin = {}
    control_k = get_k(0, x1, data_list)
    ...
    while True:
        ...
        while x2 <= max_dim - 1:  # TODO
            if x0==0:
                ...
                vis_k, vis_k_next, ind = is_cluster_found(
                    0, x1, x2, data_list, max_dim, control_k)
...
def draw_pic(par_data_list):
    ...
    for i in range(0,len(d_cluster_chain)-1):
        ...
        y_start = par_data_list[x_start]
        y_next = par_data_list[x_next]
...
if ONLY_NORMAL:
    ...
    while fi<0.2:
        ...
        while start_point < max_dim - 1:
            ...
            next_point, last_x, last_len = build_graph_with_clusters(start_point, data_list, False)
            draw_pic(data_list)
            plt.show()
            # print("d_cluster_chain = ", d_cluster_chain)
            ...
            if cond:
                ...
                #start_point = 0
                print('Стабилизация! start_point={}'.format(start_point))
                break
            else:
                start_point = a
            ...
            if(DEBUG1): log.debug("000***first={} last={} max_dim={} last_len={}".format(start_point, last_x, max_dim, last_len))
        # st.stemplot(new_ind, data_list, suptitle="Normal distribution", title='fi = {}'.format(fi),
        #             pdfname='fi{}.pdf'.format(fi), SAVEPP=True)

        # plt.show()
        if DEBUG1:
            Density=len(d_cluster_chain) / len(data_list)
            dens.append(Density)
