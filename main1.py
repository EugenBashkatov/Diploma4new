...
def change_input_data(switch,par_size = 10, par_scale = 5, ploting = False):
    ...
    if switch == "daily-10":
        df = pd.read_csv("daily-min-temperatures-test-10.csv",
                         names=['Date', 'MinTemp'])
        data = df.to_numpy()
        max_dim = sum(1 for my_line in open("daily-min-temperatures-test-10.csv", 'r'))
    if switch == "daily-30":
...
data_list = change_input_data("daily-10")[0]


max_dim = change_input_data("daily-10")[1]
